...
-- Volcando estructura para tabla classicmodels.cambios
CREATE TABLE IF NOT EXISTS `cambios` (
  ...
  `fecha_de_eliminacion` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Volcando datos para la tabla classicmodels.cambios: ~0 rows (aproximadamente)
...
INSERT INTO `offices` (`officeCode`, `city`, `phone`, `addressLine1`, `addressLine2`, `state`, `country`, `postalCode`, `territory`) VALUES
	('1', 'Esmeraldas', '+593 650 219 4782', '100 Market Street', 'Suite 300', 'CA', 'Ecuador', '94080', 'NA'),
	('2', 'Quito', '+593 215 837 0825', '1550 Court Place', 'Suite 102', 'MA', 'Ecuador', '02107', 'NA'),
	('3', 'Cuenca', '+593 212 555 3000', '523 East 53rd Street', 'apt. 5A', 'NY', 'Ecuador', '10022', 'NA'),
	('4', 'Bogota', '+57 14 723 4404', '43 Rue Jouffroy D\'abbans', NULL, NULL, 'Colombia', '75017', 'EMEA'),
	('5', 'Lima', '+51 33 224 5000', '4-1 Kioicho', NULL, 'Chiyoda-Ku', 'Peru', '102-8578', 'Japan'),
...
-- Volcando estructura para vista classicmodels.productos_pedidos_por_cliente
-- Creando tabla temporal para superar errores de dependencia de VIEW
CREATE TABLE `productos_pedidos_por_cliente` (
	`productName` VARCHAR(70) NOT NULL COLLATE 'latin1_swedish_ci',
	`customerName` VARCHAR(50) NOT NULL COLLATE 'latin1_swedish_ci',
	`officeCode` VARCHAR(10) NOT NULL COLLATE 'latin1_swedish_ci',
	`city` VARCHAR(50) NOT NULL COLLATE 'latin1_swedish_ci',
	`phone` VARCHAR(50) NOT NULL COLLATE 'latin1_swedish_ci',
	`addressLine1` VARCHAR(50) NOT NULL COLLATE 'latin1_swedish_ci',
	`addressLine2` VARCHAR(50) NULL COLLATE 'latin1_swedish_ci',
	`state` VARCHAR(50) NULL COLLATE 'latin1_swedish_ci',
	`country` VARCHAR(50) NOT NULL COLLATE 'latin1_swedish_ci',
	`postalCode` VARCHAR(15) NOT NULL COLLATE 'latin1_swedish_ci',
	`territory` VARCHAR(10) NOT NULL COLLATE 'latin1_swedish_ci'
) ENGINE=MyISAM;
...
-- Volcando estructura para disparador classicmodels.after_actualizar_employee
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `after_actualizar_employee` AFTER UPDATE ON `employees` FOR EACH ROW Begin
...
-- Volcando estructura para disparador classicmodels.after_employee_eliminar
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `after_employee_eliminar` BEFORE DELETE ON `employees` FOR EACH ROW BEGIN
    INSERT INTO exEmployees(employeeNumber, lastName, firstName, extension, email, officeCode, reportsTo, jobTitle)
...
DROP TABLE IF EXISTS `productos_pedidos_por_cliente`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `productos_pedidos_por_cliente` AS select `pro`.`productName` AS `productName`,`c`.`customerName` AS `customerName`,`off`.`officeCode` AS `officeCode`,`off`.`city` AS `city`,`off`.`phone` AS `phone`,`off`.`addressLine1` AS `addressLine1`,`off`.`addressLine2` AS `addressLine2`,`off`.`state` AS `state`,`off`.`country` AS `country`,`off`.`postalCode` AS `postalCode`,`off`.`territory` AS `territory` from (((((`customers` `c` join `employees` `emp`) join `offices` `off`) join `products` `pro`) join `orderdetails` `ordt`) join `orders` `orde`) where ((`off`.`officeCode` = `emp`.`officeCode`) and (`emp`.`employeeNumber` = `c`.`salesRepEmployeeNumber`) and (`c`.`customerNumber` = `orde`.`customerNumber`) and (`orde`.`orderNumber` = `ordt`.`orderNumber`) and (`ordt`.`productCode` = `pro`.`productCode`)) order by `c`.`customerName`;
